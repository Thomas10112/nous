/* ------------------------------------------------------------------
   ...
   Idée retenue par le jury de conception (ADR-010) : le bandeau dit le rythme
   d'une journée, mais il ne répond pas à la question qu'on se pose vraiment en
   ...
   Port fidèle de spike/week-grid-web/src/domain/phrase.js.
   ------------------------------------------------------------------ */

import { SLOTS_PER_DAY, formatSlot, type SlotWindow } from './time'
...
/** Une ligne, jamais deux, toujours vraie. */
export function phraseOfDay(windows: readonly SlotWindow[]): string {
  const w = [...windows]
    .filter((x) => x.endSlot > x.startSlot)
    .sort((a, b) => a.startSlot - b.startSlot)
  if (w.length === 0) return 'la journée est à vous'

  const first = w[0]!.startSlot
 ...
 */
export function inkWeight(count: number): 'vide' | 'leger' | 'charge' {
