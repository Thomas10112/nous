...
| ----- | ---------------- |
| **Auto Blocker** (Samsung) | S'il est actif, **aucun APK ne s'installe**, ni depuis Chrome ni depuis Obtainium, et le menu « Installer des applications inconnues » est lui-même grisé. C'est le point de défaillance unique de tout le plan Android. Il est activé d'usine seulement sur les appareils livrés en One UI 6.1.1 ou plus récent ; le S24 Ultra est sorti en One UI 6.1, donc il est probablement inactif — **à vérifier avant tout le reste**, une réinitialisation ou un transfert Smart Switch ayant pu le rallumer. One UI 8.5 ajoute une option de **réactivation automatique 30 minutes après une désactivation** : elle est facultative, on peut toujours laisser Auto Blocker éteint. Il bloque aussi les commandes par câble USB, donc il **neutralise `adb install`** — le repli et le verrou ferment la même porte. |
| **`USE_EXACT_ALARM` au lieu de `SCHEDULE_EXACT_ALARM`** | Décision d'architecture. La raison la mieux établie n'est pas un bug mais la documentation d'Android 14 : `SCHEDULE_EXACT_ALARM` est **refusée après un transfert par sauvegarde/restauration**, même si elle avait été accordée. Le jour où l'un des deux change de téléphone avec Smart Switch, tous les rappels de Nous casseraient en silence. `USE_EXACT_ALARM` est une permission normale, accordée à l'installation, non révocable, insensible au transfert ; elle convient à une app de calendrier, et la politique du Play Store qui la réserve ne s'applique pas puisque Nous ne passe pas par Play. (Un signalement de 2024 sur la série S24 décrit en plus la disparition définitive du réglage après un refus ; non confirmé depuis, à traiter comme un risque, pas comme un fait.) |
| **Advanced Protection** (Android 16) | Second interrupteur, aussi fatal : quand il est actif, la permission « installer des applications inconnues » **ne peut plus être accordée** (grisée), et les mises à jour des apps déjà installées hors magasin cessent. Il n'est pas actif par défaut mais Google le recommande activement. À inscrire, comme Auto Blocker, dans les choses à ne pas activer. |
| **Identity Check** | S'il est activé (protection contre le vol de Samsung, **désactivé par défaut**), couper Auto Blocker hors d'un lieu de confiance réclame la biométrie. Ce n'est pas une contrainte par défaut, c'est un mode de panne à connaître. |
...
| **Résolution et fluidité** | Le téléphone sort en FHD+ (2340 × 1080), pas en QHD+ ; Android ajuste la densité en conséquence, si bien que la géométrie en `dp` bouge sans doute peu — à **mesurer** (`Dimensions.get('window')`, `PixelRatio.get()`) dans les deux modes plutôt qu'à supposer. En revanche le réglage **« Fluidité des mouvements »** compte vraiment : en mode Standard, l'écran est plafonné à 60 Hz et le budget redevient 16,7 ms, ce qui invalide le seuil de 8,3 ms. Relever le mode avant de mesurer. |
...
| **Permissions à ne jamais déclarer** | Play Protect ne fait pas qu'avertir : il **bloque durement**, sans bouton « installer quand même », les APK hors magasin qui demandent `RECEIVE_SMS`, `READ_SMS`, l'écoute de notifications ou l'accessibilité. Aucune de ces quatre permissions ne doit apparaître dans le manifeste de Nous, même par une dépendance. Les « paramètres restreints » d'Android 15 se sont par ailleurs élargis (superposition, accès aux données d'usage, administrateur d'appareil) : ne pas concevoir de rappel en superposition plein écran. |
| **Rafales de messages** | Android 16 baisse progressivement le volume des notifications successives d'une même app pendant jusqu'à deux minutes. Les conversations prioritaires en sont exemptées : les messages du partenaire doivent être publiés en **MessagingStyle adossé à un raccourci de conversation**, sinon le deuxième message d'une rafale est muet. |
| **Priorité FCM** | Google plafonne les messages en priorité haute par application et déclasse au-delà. Réserver la priorité haute à ce qui produit une notification visible (message, proposition) et laisser la synchronisation en priorité normale. |
| **Device Care** | L'optimisation quotidienne automatique et le redémarrage programmé de Samsung tuent des processus indépendamment de la liste « jamais en veille », souvent la nuit — exactement quand un rappel matinal est armé. À vérifier dans Device Care. |
| **One UI 9 / Android 17 imminent** | La bêta est sur S24 depuis le 2 septembre 2026, la version stable est attendue fin septembre. **Ne pas livrer la première version de Nous pendant la fenêtre de déploiement** ; viser `compileSdk` 37 et prévoir une reprise de tests après la montée. Android 17 durcit aussi le son en arrière-plan : un rappel avec son personnalisé exigera `USE_EXACT_ALARM` **et** un flux marqué `USAGE_ALARM`. |
| **Widgets d'écran de verrouillage** | One UI 8.5 accepte les widgets tiers sur l'écran verrouillé : une seconde surface gratuite pour « le prochain événement », à explorer en Phase 16. |
...
| **Abonnements qui expirent** | Des endpoints iOS deviennent invalides au bout d'une à deux semaines, et réinstaller la web app détruit l'abonnement. Pire : `pushsubscriptionchange` **n'est pas implémenté sur iOS**, donc l'app n'est jamais prévenue, et se réabonner exige un geste de l'utilisateur. Il faut comparer l'endpoint à celui enregistré **à chaque lancement** et afficher un bouton « Réactiver les notifications ». Sans cela, ses rappels s'arrêtent en silence et personne ne le sait. |
| **Retard des notifications sur téléphone verrouillé** | Au-delà d'une dizaine de minutes de veille, les notifications web ne sont pas délivrées et tombent d'un coup au déverrouillage. Mesuré à 9–10 minutes sur un iPhone en iOS 26.4.1, signalé aussi sur iOS 18.7 et 26.0, reconnu par un ingénieur Apple, sans correctif. **À dire au couple sans détour** : un message envoyé pendant qu'elle a le téléphone posé peut n'apparaître qu'à la reprise en main. C'est la limite la plus concrète du client iPhone. |
| **Vidéo cassée en mode web app** | Depuis iOS 26.0.1, la lecture vidéo et audio est signalée bloquée dans les web apps installées — la vignette s'affiche, la lecture ne démarre pas — alors que tout fonctionne dans l'onglet Safari. Sans réponse d'Apple. Conséquence pour la galerie : **tester la lecture d'une vidéo sur son téléphone avant de promettre la fonctionnalité**, et prévoir l'ouverture dans Safari en repli. |
| **Photos et appareil photo** | En mode web app, l'autorisation caméra n'est pas conservée et est redemandée à chaque fois, la caméra cesse de fonctionner après un passage en arrière-plan, et l'envoi d'une photo peut recharger la page — donc perdre ce qui était en cours. L'ajout de photo côté iPhone doit passer par le sélecteur de fichiers, jamais par un flux caméra long, et l'état doit être sauvegardé avant l'envoi. |
...
**la PWA sert aussi sur Android**. Une web app installée par Chrome ne dépend d'aucun de ces
verrous et reçoit du Web Push. Ce que l'application installée apporte en plus se réduit à
trois choses : le widget d'écran d'accueil, les rappels exacts hors ligne, et le geste à
