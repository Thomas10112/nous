...
| Réutilisation du code v1 | Logique, types, tokens, icônes, contrats : **≈ 30 %** ; tout le rendu est réécrit | **≈ 0 %** de code, seulement les idées | **≈ 95 %** tel quel, mais avec les dettes D13–D19 à corriger dans le CSS |
...
| Écosystème 2026 | Expo SDK 56 (mai 2026), Expo Router, `@expo/ui` (SwiftUI/Compose) | Flutter 3.x stable | Capacitor 7 |

## 2. Notation sur les seize critères (1 = faible, 5 = excellent)

| # | Critère | A. RN + Expo | B. Flutter | C. Hybride | Commentaire |
| - | ------- | :-: | :-: | :-: | ----------- |
| 1 | Offline-first (base locale, requêtes réactives, outbox) | 4 | 5 | 2 | A : SQLite natif + Drizzle, moteur maison (ADR-005). C : IndexedDB en WebView iOS peut être purgé ; SQLite via plugin = pont JS coûteux. |
...
| 4 | Widgets système | 3 | 3 | 2 | Natif dans tous les cas ; A a des ponts prêts (apple-targets, android-widget). C : rien n'est partagé avec l'app. |
| 5 | Performances listes/grilles | 4 | 5 | 2 | A : FlashList, vues natives, Hermes v1. C : DOM dans WebView, jank sur Android modeste. |
| 6 | Animations, gestes, reduced-motion | 4 | 5 | 2 | A : Reanimated worklets sur thread UI, `ReduceMotion` intégré. |
| 7 | Calendrier tactile (grille 30 min, drag, resize, multi-jours) | 4 | 4 | 2 | Aucune bibliothèque ne convient à l'identité Nous (ADR-003) ; A et B écrivent la même logique ; C se bat contre le scroll du navigateur. |
| 8 | Stockage local (SQLite, MMKV, fichiers, cache images) | 4 | 5 | 2 | |
| 9 | iOS (build, TestFlight, distribution à 2) | 4 | 4 | 4 | EAS simplifie A ; les trois ont besoin d'un compte développeur. |
| 10 | Android (build, APK) | 5 | 5 | 4 | |
| 11 | Réutilisation du TypeScript existant | 4 | 1 | 5 | A garde types, dates, utils, login, tokens, icônes, contrats de props, moteur de geste (logique). |
| 12 | Identité visuelle Nous (Fraunces/Inter, tokens, grain, palettes) | 4 | 4 | 5 | A : tokens en TS, polices embarquées, grain en tuile, palettes animées. |
| 13 | Maintenabilité pour un dev seul + IA | 5 | 2 | 3 | A : un langage, un écosystème très documenté, IA très efficace en TS/RN. B : deux langages. C : deux mondes (web + natif) et une dette CSS mobile à porter. |
| 14 | Vidéo (lecture, compression, upload) | 4 | 4 | 2 | A : expo-video, react-native-compressor. |
| 15 | Messagerie + présence | 5 | 5 | 4 | |
| 16 | Desktop / web plus tard | 4 | 3 | 5 | A : Expo web possible, et `packages/domain` sert au web actuel. |
| | **Total /80** | **68** | **64** | **52** | |

Le total ne décide pas seul : les critères qui **tranchent** pour ce projet sont 7
(calendrier tactile), 11 (réutilisation), 13 (dev seul) et 12 (identité). Sur ces
quatre, A domine ; B ne perd que sur 11 et 13, mais y perd lourdement ; C gagne 11 et
12 mais échoue sur 7, 5 et 6 — et le brief exclut explicitement « un site web rendu
responsive ».
...
- Expo SDK 56 (React Native 0.85, React 19.2, New Architecture, Hermes v1), Expo Router ;
