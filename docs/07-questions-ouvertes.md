...
| T10 | **Appareils réels** du couple : modèles, versions d'OS, taux de rafraîchissement. | **Répondu le 07/09** : lui **Samsung Galaxy S24 Ultra** (écran 120 Hz adaptatif → budget par frame ≈ 8,3 ms, pas 16,7), elle **iPhone 16** (modèle de base, écran 60 Hz → budget 16,7 ms). Versions d'OS exactes encore à relever ; le spike les mesure lui-même. | 1 |
...
| T13 | **Lequel de vous a un iPhone ?** | **Répondu le 07/09 : sa copine, iPhone 16.** C'est donc le **cas B** d'ADR-009 : app React Native sur le S24 Ultra, **PWA sur l'iPhone 16**. La variante iPhone de la roadmap (§7, +25 à 35 jours) s'applique. Version d'iOS à confirmer (Web Push ≥ 16.4, Declarative Web Push ≥ 18.4, glissé web fiable ≥ 26.5). | 1 |
