...
| Q6 | **Vidéos** : limite de durée/taille ? | **50 Mo par fichier tant que le plan Supabase est gratuit** (≈ 1 min 30 en 1080p ; 720p proposé pour 3 min), 200 Mo seulement après passage au Pro ; constante `media.maxUploadBytes` dérivée du plan, au-delà on propose de recomprimer ou de couper. | 11 |
...
| # | Question | Défaut retenu | Phase |
| - | -------- | ------------- | ----- |
| T1 | Le projet Supabase `nous` (eu-west-3, créé le 14/08/2026, organisation sur le plan gratuit) est **en pause** (`INACTIVE`). Un projet en pause se restaure en un clic pendant **90 jours** seulement (ensuite : téléchargement de sauvegarde). Pendant la pause, le site web v1 est indisponible. | **Restauré le 06/09/2026** (`ACTIVE_HEALTHY`) ; `keep-alive` qui touche une table dès la Phase 2. | 1 |
...
| T5 | Widgets iOS : `expo-widgets` (stable depuis SDK 56, images via `widgetsDirectory`, TSX sans Swift) ou SwiftUI natif via `@bacons/apple-targets` ? | `expo-widgets` en voie principale ; SwiftUI seulement si le rendu Fraunces / crème n'est pas atteignable **et** qu'un Mac est disponible. Le snapshot (ADR-006) est le même dans les deux cas. | 16 |
...
| T11 | Un **Mac** est-il disponible ? Conséquences réelles : sans Mac, pas de simulateur iOS donc pas de Maestro iOS ni de test de push en simulateur ; `@bacons/apple-targets` exige Xcode ; chaque essai Swift = un build EAS (quota gratuit : 15 iOS + 15 Android par mois). Avec un Mac (ou des runners macOS GitHub Actions, ≈ 200 min/mois sur un dépôt privé) : Maestro iOS et Xcode possibles. | Non supposé : iOS testé par check-list manuelle, widgets iOS en `expo-widgets` seulement, Swift hors périmètre. À revoir si un Mac apparaît. | 1, 5, 16, 18 |
| T12 | **Projet Firebase** (gratuit) pour les pushs Android : `google-services.json`, clé de compte de service FCM V1 ; clé APNs pour iOS ; `EXPO_ACCESS_TOKEN` pour la fonction Edge. | À créer avant la Phase 15. | 15 |
| T9 | **Plan Supabase** : le gratuit met le projet en pause après 7 jours sans requête, limite le Storage à 1 Go (photos v1 + vidéos) et réserve le *branching* au plan Pro (25 $/mois). Rester gratuit avec un `keep-alive` et des tests locaux, ou passer Pro ? | Gratuit + keep-alive + tests locaux jusqu'à la Phase 11 ; décision Pro avant les vidéos. Volume mesuré le 06/09 : **4,4 Ko de données, 435 Ko de photos** ([08](08-mesures-phase1.md)) — le gratuit suffit jusqu'aux vidéos. | 1, 11, 19 |
